#!/usr/bin/env python3
"""
Training script for DeAltHDR model with dual encoder and mixed training strategy
"""
...
# for deblurring/deraining/etc. comment the line above, and uncomment the next line.
...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            train_set = VideoImageDataset(opt, phase)
            ...
            val_set = VideoImageDataset(opt, phase)
            val_loader = create_dataloader(
                ...
                seed=opt['manual_seed'])
            logger.info(
                f'Number of val images/folders in dataset: '
                f'{len(val_set)}')
        else:
...
def main():
    ...
    while current_iter <= total_iters:
        ...
        if dist.get_rank() == 0:
            print(f"Loss at the end of Epoch {epoch} is {model.get_current_log()['l_pix']}.") 
        # end of iter
        epoch += 1
